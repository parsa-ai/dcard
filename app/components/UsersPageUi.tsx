...
import { Card, User } from '@/lib/type';
...
export default function UsersPageUi({ usersData }: { usersData: User[] }) {
        ...
        <div className="space-y-6">
            ...
            <div className="hidden md:block overflow-x-auto border border-gray-100 rounded-xl">
                <table className="w-full text-center border-collapse">
                    ...
                    <tbody>
                        {usersData.map((user) => {
                            let cards: Card[] = JSON.parse(user.cards)
                            ...
                            return (
                                <tr key={user.id} className="border-b border-gray-100 transition-colors hover:bg-gray-50">
                                    ...
                                    <td className="p-4">
                                        <button onClick={() => setSelectedUserCards(cards)} className="text-blue-900 hover:underline mx-auto flex items-center gap-1">مشاهده</button>
                                    </td>
                                    <td className="p-4">
                    ...
                    return (
                        <div key={user.id} className="bg-white border border-gray-100 rounded-xl p-4 shadow-sm">
                            ...
                            <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-50">
                                <button
                                    onClick={() => setSelectedUserCards(cards)}
                    ...
                    <div className="bg-white rounded-2xl max-w-6xl  w-full p-6 shadow-[0_0_20px] shadow-blue-500/30 -z-20">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 max-h-130 overflow-y-auto " dir='ltr'>
                            {selectedUserCards.map((card, index) => {
                                const bankIcon = detectBank(card.pan, banks);
                                ...
                                return (
                                    <div key={index} className='flex items-end justify-end relative min-h-70 md:min-h-70 xl:min-h-51 '>
                                        ...
                                        <div className='m-4 w-full  text-black/70'>
                                            <p className='w-full text-2xl xl:text-xl  text-center space-x-3.5 tracking-[0.4rem] mb-8 font-semibold'>{card?.pan}</p>
                                            <div className='flex justify-between w-full'>
                                                <p>CVV2: {card?.cvv2 || "نامشخص"}</p>
                                                <p>انقضا: {card?.exMonth}/{card?.exYear}</p>
                                            </div>
                                        </div>
                                    </div>
                                )
